...
void solve()
{
    ...
    fl(i,q)
    {
        int x;
        cin >> x;
        cout << pf[upper_bound(a.begin(), a.end(), x) - a.begin()] << " ";
    }
    cout nl;
}
...
If Timur's legs have length 1
, then he can only climb stair 1
, so the highest he can reach is 1
 meter.
If Timur's legs have length 2
 or 4
, then he can only climb stairs 1
, 2
, and 3
, so the highest he can reach is 1+2+1=4
 meters.
If Timur's legs have length 9
 or 10
, then he can climb the whole staircase, so the highest he can reach is 1+2+1+5=9
 meters.
